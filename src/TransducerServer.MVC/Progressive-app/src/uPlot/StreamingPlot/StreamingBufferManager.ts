...
export class PlotBufferManager {
  ...
  constructor(
    rangeGetter: () => [number, number],
    config: PlotBufferManagerConfig
  ) {
  ...
  public get Source() {
    ...
    return frameIndex1 > frameIndex2
      ? this.frames2[frameIndex2]
      : this.frames[frameIndex1];
  }
  ...
  private SetValue(index: number, segmentIndex: number, value: SeriesValue) {
    ...
    if (indexInFrame2 >= 0) {
      let frameIndex2 = Math.trunc(
        (index - this.frameSize / 2) / this.frameSize
      );
  ...
  private HandleFramesBufferExpand(expandToTime: number) {
    ...
    let framesBufferIndex2 =
      this.tickToGridIndex(expandToTime) - this.frameSize / 2;
  ...
  HandleFramesBuffer = (
    framesBuffer: AlignedData[],
    expandToIndex: number,
    getLastTime: () => number
  ) => {
