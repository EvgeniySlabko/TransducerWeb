import {
  ...
  PlotChannel,
} from "../../Channel/Channel/PlotChannel";
...
export class MyUPlot extends MyUPlotBase {
  ...
  private HandleData = (channel: PlotChannel, args: PlotChannelDataArgs) => {
    var curIndex =
      this.traces.find((c) => c.channel == channel)!.seriesInfo
        .dataBufferIndex - 1;
  ...
  private ScaleHandler = () => {
    ...
    if (
      maxTime >
        max - (this.params.screenSize() * this.screenOffset) / 100 + 0.03 &&
      this.streaming
    ) {
  ...
  private BuildNewPlot = (channels: PlotChannel[]) => {
    ...
    this.bufferManager = new PlotBufferManager(
  ...
  protected SeriesDraw(i: number) {
    ...
    if (channel != undefined) {
      ...
      this.plot!.series[i].points!.stroke = () =>
        existsChannel.channel.Style.color;
    }
  }
  ...
  protected Wheel(e: any): boolean {
    if (this.streaming) {
      ...
      let newSize =
        this.params.screenSize() + this.params.screenSize() * dw * 0.1;
