...
export class MyUPlotViewer extends MyUPlotBase {
  ...
  public FromSnapshot(snapshot: Snapshot) {
    ...
    this.buffer = new LogLevelBuffer(() => [
      this.params.range[0],
      this.params.range[1],
    ]);
    this.buffer.FromSnapshot(snapshot);
    var styles = trackData.map((t) => t.style);
    this.params.t0 = Math.min(
      ...trackData
        .map((trackData) => trackData.data)
        .map((data) => (data.time.length > 0 ? data.time[0] : 0))
    );
    this.params.th = Math.max(
      ...trackData
        .map((trackData) => trackData.data)
        .map((data) =>
          data.time.length > 0 ? data.time[data.time.length - 1] : 100
        )
    );
