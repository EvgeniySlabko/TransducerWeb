...
export class Navbar extends React.Component<Props, IState> {
  ...
  private disableStartClick = () =>
    (!this.props.enable && !this.props.reportVieving) ||
    this.state.disableStart;
  private disableClearClick = () =>
    !this.props.enable || this.state.disableStart;
  private disableAddClick = () =>
    this.props.streaming || this.props.reportVieving;
  private disableSaveClick = () =>
    this.props.streaming ||
    this.props.reportVieving ||
    !this.props.thereAreDataForSaving;
  ...
  render() {
    return (
      <div className="nav-tab-container">
        <Button
          title="Начать измерение. (Space)"
          size="large"
          id="Start"
          shape="default"
          disabled={this.disableStartClick()}
          icon={
            this.props.reportVieving ? (
              <ArrowLeftOutlined />
            ) : this.props.streaming ? (
        ...
        <Button
          title="Очистить результаты / Окончить эксперимент. (C)"
