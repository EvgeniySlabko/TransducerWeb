...
export class CellsGroup extends React.Component<Props, IState> {
  ...
  setZeroClick = () => {
    ...
    notification.success({
      message: `Смещение установлено для датчика ${
        this.props.group.node.fullSensorInfo.SensorType
      } - ${currentOffset.toFixed(
        this.props.group.node.fullSensorInfo.Accuracy
      )}${this.props.group.node.fullSensorInfo.UnitValueName}`,
      duration: 2,
    });
  };
  ...
  setChannelVisibilty = (channelindex: number, value: boolean) => {
    this.props.group.channelsInfo.channelGroups[
      channelindex
    ].cellChannel.Style.visible = value;
    this.setState({});
  };
          ...
          header={
            <>
              ...
              <div className="vertical-flex">
                <h6 className="cell-group-title">
                  {this.props.group.node.fullSensorInfo.SensorType}
                </h6>
                <h6 className="cell-group-title">
                  ID: {this.props.group.node.fullSensorInfo.SensorId}
                </h6>
              </div>
