...
import {
  AllChannelsInfo,
  CreateAllChannels,
} from "../Channel/AllChannelsFactory";
...
import {
  SensorController,
  SensorControllerArgs,
} from "../Sensor/SensorsManager/SensorsManager";
import { SensorWorker } from "../Sensor/SensorWorker";
import {
  ApplayLocalStorageSettingsForGroups,
  ApplySensorParameters as ApplaySensorStorageParameters,
...
export class App extends React.Component<Props, IState> {
  ...
  constructor(props: Props) {
    ...
    this.props.sensorService.onDispatch.addListener(
      "Add",
      this.newSensorHandler
    );
  }
  ...
  sensorCloseHandler = async (sensorWorker: SensorWorker, args: string) => {
    let index = this.state.groups.findIndex(
      (g) => g.node.worker == sensorWorker
    );
  ...
  newSensorHandler = async (args: SensorControllerArgs) => {
    if (this.state.plotsManager) {
      ...
      ApplayLocalStorageSettingsForGroups(
        allChannelsInfo.channelGroups,
        args.fullSensorInfo.SensorId
      );
      ...
      allChannelsInfo.PeackDetectedEvent.sub((channel, peakArgs) => {
        let group = allChannelsInfo.channelGroups.find(
          (g) => g.plotChannel === channel
        );
        ...
        this.state.plotsManager?.AddLabelForChannel({
          ...
          time: peakArgs.time,
          text:
            peakArgs.peakValue.toFixed(group?.cellChannel.Style.accuracy) +
            " " +
            group?.cellChannel.Style.unitsName,
          value: peakArgs.peakValue,
        });
      });

      //this.setState((prev, props) => ({}));
      this.state.plotsManager.AddChannels(
        allChannelsInfo.channelGroups.map((g) => g.plotChannel)
      );
      let recodingChannels: RecordigGroup[] = this.state.groups.map((g) => {
        return {
          savingChannels: g.channelsInfo.channelGroups.map(
            (cg) => cg.savingChannel
          ),
  ...
  openFileHandler = async (file: File) => {
    ...
    if (!this.state.viewingReport)
      this.state.groups.forEach(async (g) =>
        this.props.sensorService.RemoveSensor(g.node.worker)
      );
