...
import {
  FilterType,
  GetSensorParameters,
  SaveChannelGroupParameters,
  SaveSensorParameters,
  SensorStorageParameters,
} from "../../Storage/ChannelsDataStorage";
...
export class SensorSettingsTab extends React.Component<Props, IState> {
  ...
  async componentDidMount() {
    try {
      let filterParameters =
        this.props.group.channelsInfo.getFilterParameters();
      let holdingRegisters =
        await this.props.group.node.worker.GetHoldingRegisters();
      let sensorparameters = await GetSensorParameters(
      ...
      this.setState(() => ({
        ...
        visibleChannels: this.props.group.channelsInfo.channelGroups
          .map((c) => c.cellChannel)
          .map((ch) => [ch.Style.valueName, ch.Style.visible]),
  ...
  onOk = async () => {
    this.props.group.channelsInfo.setPeackAnalizerState(
      this.state.trackMaximum
    );
    ...
    try {
      ...
      await this.props.group.node.worker.SetExternalSpeedSensorState(
        this.state.externalSpeedSensor
      );
      ...
      this.props.group.channelsInfo.setInvertiorSourceState(
        this.state.invertion
      );
    ...
    for (let i = 0; i < this.state.visibleChannels.length; i++)
      this.props.setChannelVisibilty(i, this.state.visibleChannels[i][1]);
  };
  ...
  onFilterEnabledChanged = (value: boolean) =>
    this.setState(() => ({ enabled: value }));
  onFilterFcChanged = (value: number) => this.setState(() => ({ fc: value }));
  onFilterTypeChanged = (value: FilterType) =>
    this.setState(() => ({ filterType: value }));
  onFilterOrderChanged = (value: number) =>
    this.setState(() => ({ order: value }));

  onInvertionChanged = (value: boolean) =>
    this.setState(() => ({ invertion: value }));
  onAbsoluteChanged = (value: boolean) =>
    this.setState(() => ({ absolute: value }));
  ...
  onExternalSpeedSensorChanged = (value: boolean) =>
    this.setState(() => ({ externalSpeedSensor: value }));
  onTrackMaximumChanged = (value: boolean) =>
    this.setState(() => ({ trackMaximum: value }));
  onSpeedPeriodChanged = (value: number) =>
    this.setState(() => ({ speedPeriod: value }));
      ...
      <Modal
        ...
        footer={[
          <Button
            ...
            icon={<SaveOutlined onClick={this.onSaveParamsToStorage} />}
          />,
          ...
          <Tabs defaultActiveKey="1">
            <TabPane tabKey="1" tab="Общие" key="1">
              <SensorParameters
                ...
                onSpeedPeriodChanged={this.onSpeedPeriodChanged}
                onTrackMaximumChanged={this.onTrackMaximumChanged}
                onVisibleChannelsChanged={this.onVisibleChannelsChanged}
              ></SensorParameters>
            </TabPane>
            <TabPane tabKey="2" tab="Фильтр" key="2">
              <FilterSettings
                ...
                order={this.state.order}
                fc={this.state.fc}
                onFilterEnabledChanged={this.onFilterEnabledChanged}
                onFilterFcChanged={this.onFilterFcChanged}
                onFilterOrderChanged={this.onFilterOrderChanged}
