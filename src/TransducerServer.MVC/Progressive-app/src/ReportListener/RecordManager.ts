  ...
  public SetChannels(groups: RecordigGroup[]) {
    ...
    this.recordingGroups.forEach((g) =>
      g.sensorWorker.onClose.sub((sensor) => {
        let index = this.recordingGroups.findIndex(
          (rg) => rg.sensorWorker === sensor
        );
    ...
    allPlotChannels = allPlotChannels.concat(
      ...groups.map((g) => g.savingChannels)
    );
    this.listener.SetChannels(allPlotChannels);
  }

  public async StartListening() {
    this.currentMinAvg = await GetMinAvgFactor(
      this.recordingGroups.map((g) => g.sensorWorker)
    );
