    ...
    public async ToCSV(stream: FileSystemFileHandle) {
        ...
        let addRow = (title: string, data: SeriesValue[]) => {
            ...
            for (let i = 0; i < data.length; i++) {
                let strVal = data[i] === null || data[i] === undefined ? "" : (<number>data[i]).toString();
        ...
        for (let i = 1; i < alignedData.length; i++) addRow(this.data[i - 1].style.legendTitle, alignedData[i]);
