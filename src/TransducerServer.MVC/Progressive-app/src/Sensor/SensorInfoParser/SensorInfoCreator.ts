...
let count = 0;
export async function GetFullSensorInfo(
  sensor: SensorWorker
): Promise<SDefs.FullSensorInfo> {
...
export async function CreateFullSensorInfo(
  serviceInfo: SDefs.SensorSK,
  ...
  fullInfo.SensorId =
    formatDigit(serviceInfo.ID[0]) +
    formatDigit(serviceInfo.ID[1]) +
    formatDigit(serviceInfo.ID[2]);
  ...
  switch (Index_Opis) {
    case 0: { // Момент
      ...
      if (
        holdingRegisters.flags.Pronometer &&
        holdingRegisters.flags.ControlButton
      ) {
