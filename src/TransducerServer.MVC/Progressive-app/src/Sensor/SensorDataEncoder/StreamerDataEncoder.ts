...
import {
  CalculateTime,
  ISensorStreamerDataEncoder,
  SreamigSensorDataHeader,
} from "./ISensorStreamerDataEncoder";
import { ModBusSensorDataCommandEncoder } from "./ModBusSensorDataEncoder";

export class SensorStreamerDataEncoder
  extends ModBusSensorDataCommandEncoder
  implements ISensorStreamerDataEncoder
{
  ...
  async GetTorque(avgRatio: number, currentTime: number): Promise<SensorData> {
    ...
    let allBytes = new Uint8Array(
      this.headerBytes.length + datatorque.length + 1
    );
  ...
  async GetSpeed(currentTime: number): Promise<SensorData> {
    ...
    let crc = this.ReadCRC();
    let allBytes = new Uint8Array(
      this.headerBytes.length + dataSpeed.length + 1
    );
  ...
  async GetTemperature(currentTime: number): Promise<SensorData> {
    ...
    let crc = this.ReadCRC();
    let allBytes = new Uint8Array(
      this.headerBytes.length + dataTemperature.length + 1
    );
...
export function CreateStreamingSensorDataCommandEncoder(
