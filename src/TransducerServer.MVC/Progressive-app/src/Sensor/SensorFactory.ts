...
export async function CeateSensorWorker(decoderType: DecoderType) : Promise<SensorWorker>{
    ...
    switch (decoderType) {
        case "RS485":{
            ...
            return new SensorWorker(CreateRS485Sensor(ioWorker), CreateDecoderParameters(decoderType), decoderType);
        }
        case "VCOM":{
            let ioWorker = await CreateIOWorker();
            return new SensorWorker(CreateVCOMSensor(ioWorker), CreateDecoderParameters(decoderType), decoderType);
        }
        case "Faker":{
...
function CreateRS485Sensor(bufferedWorker: SerialBufferedWorker) : SingleComponentSensorExchanger
{
    //TO DO Params.
