...
} from "../SensorDataEncoder/ISensorStreamerDataEncoder";
...
export class SingleComponentSensor extends SingleComponentSensorBase {
  ...
  public async SetT0() {
    ...
    this.timeBase = CalculateTime(
      holdingRegisters.TimeLow,
      holdingRegisters.TimeHigh
    );
    super.SetT0();
  }
  ...
  protected async ProcessCommand(byte: number): Promise<boolean> {
    ...
    switch (byte) {
      case StramingPackageType.TORQUE:
        let torqueSensorData = await this.seensorDataCommandReceiver.GetTorque(
          this.avgRatio ?? 1,
          calculatedTime
        );
      ...
      case StramingPackageType.SPEED:
        let speedSensorData = await this.seensorDataCommandReceiver.GetSpeed(
          calculatedTime
        );
      ...
      case StramingPackageType.TEMPERATUR:
        let temperatureSensorData =
          await this.seensorDataCommandReceiver.GetSpeed(calculatedTime);
        if (this.avgRatio)
