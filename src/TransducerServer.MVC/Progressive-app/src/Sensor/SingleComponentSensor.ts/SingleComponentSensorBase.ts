...
import {
  FlagRegistersAddresses,
  HoldingRegisters,
  InputComplex,
  InputRegistersAddresses,
  SensorCoilValue,
  SensorCommand,
  SensorData,
  SensorMessage,
...
export class SingleComponentSensorBase implements ISingleComponentSensor {
  ...
  protected _onTorqueData = new EventDispatcher<
    ISingleComponentSensor,
    SensorData
  >();
  protected _onSpeedData = new EventDispatcher<
    ISingleComponentSensor,
    SensorData
  >();
  protected _onTmpData = new EventDispatcher<
    ISingleComponentSensor,
    SensorData
  >();
  protected _onMessage = new EventDispatcher<
    ISingleComponentSensor,
    SensorMessageEventArgs
  >();
  ...
  public async ReadInputComplex(): Promise<InputComplex> {
    let command = this.commandFactory.CreateDefaultCommand(
      SensorCommand.READ_INPUT_REGISTERS,
      InputRegistersAddresses.MainValue,
      5
    );
  ...
  public async GetHoldingRegisters(): Promise<HoldingRegisters> {
    ...
    var command = this.commandFactory.CreateDefaultCommand(
      SensorCommand.READ_HOLDING_REGISTERS,
      StorageRegistersAddresses.FLAGS,
      5
    );
  ...
  public async SetExternalSensorState(state: boolean): Promise<void> {
    ...
    await this.SendRequesAndWaitResponse<void>(
      this.commandFactory.CreateDefaultCommand(
        SensorCommand.FORCE_SINGLE_COIL,
        FlagRegistersAddresses.EXTERNAL_SPEED_SENSOR,
        state ? SensorCoilValue.COIL_ON_VALUE : SensorCoilValue.COIL_OFF_VALUE
      )
    );
  }
  ...
  public async SetUsingFloatState(state: boolean): Promise<void> {
    ...
    await this.SendRequesAndWaitResponse<void>(
      this.commandFactory.CreateDefaultCommand(
        SensorCommand.FORCE_SINGLE_COIL,
        FlagRegistersAddresses.IS_FLOAT_USING,
        state ? SensorCoilValue.COIL_ON_VALUE : SensorCoilValue.COIL_OFF_VALUE
      )
    );
  }
  ...
  public async GetSkInfo(): Promise<SensorSK> {
    ...
    var command = this.commandFactory.CreateSingleCommand(
      SensorCommand.REPORT_SLAVE_ID
    );
  ...
  public StartStreaming = async () => {
    ...
    await this.SendRequesAndWaitResponse<void>(
      this.commandFactory.CreateDefaultCommand(
        SensorCommand.FORCE_SINGLE_COIL,
        FlagRegistersAddresses.START_STREAMING,
        SensorCoilValue.COIL_ON_VALUE
      )
    );
    this._onMessage.dispatch(this, {
      msgType: SensorMessage.StartStreaming,
  ...
  public async StopStreaming() {
    ...
    var command = this.commandFactory.CreateDefaultCommand(
      SensorCommand.FORCE_SINGLE_COIL,
      FlagRegistersAddresses.START_STREAMING,
      SensorCoilValue.COIL_OFF_VALUE
    );
  ...
  public async SetAvgRatio(avgRatio: number) {
    ...
    await this.SendRequesAndWaitResponse<void>(
      this.commandFactory.CreateDefaultCommand(
        SensorCommand.PRESET_SINGLE_REGISTER,
        StorageRegistersAddresses.AVG_RATIO,
        avgRatio
      )
    );
  ...
  public SetComputerConnection = async () =>
    await this.SendRequesAndWaitResponse<void>(
      this.commandFactory.CreateDefaultCommand(
        SensorCommand.FORCE_SINGLE_COIL,
        FlagRegistersAddresses.COMPUTER_CONNECTION,
        SensorCoilValue.COIL_ON_VALUE
      )
    );
  public UnsetComputerConnection = async () =>
    this.SendRequesAndWaitResponse<void>(
      this.commandFactory.CreateDefaultCommand(
        SensorCommand.FORCE_SINGLE_COIL,
        FlagRegistersAddresses.COMPUTER_CONNECTION,
        SensorCoilValue.COIL_OFF_VALUE
  ...
  public SetSpeedPeriod = async (speedPerion: number) => {
    ...
    await this.SendRequesAndWaitResponse<void>(
      this.commandFactory.CreateDefaultCommand(
        SensorCommand.PRESET_SINGLE_REGISTER,
        StorageRegistersAddresses.SPEED_PERIOD,
        speedPerion
      )
    );
  ...
  public async StopMeasuring() {
    ...
    var command = this.commandFactory.CreateDefaultCommand(
      SensorCommand.FORCE_SINGLE_COIL,
      FlagRegistersAddresses.START_MEASURING,
      SensorCoilValue.COIL_OFF_VALUE
    );
    await this.SendRequesAndWaitResponse<void>(command);
  }
  ...
  private async ProcessDecoderCommands(command: number): Promise<boolean> {
    switch (command) {
      case SensorCommand.FORCE_SINGLE_COIL: {
        let registers =
          await this.sensorDataCommandReceiver.GetSingleCoilAnswer();
        //console.log(registers);
        this.DispatchCommandListener(
          SensorCommand.FORCE_SINGLE_COIL,
          registers
        );
        return true;
      ...
      case SensorCommand.PRESET_SINGLE_REGISTER: {
        let registers =
          await this.sensorDataCommandReceiver.GetPresetSingleRegisterAnswer();
        //console.log(registers);
        this.DispatchCommandListener(
          SensorCommand.PRESET_SINGLE_REGISTER,
          registers
      ...
      case SensorCommand.PRESET_MULTIPLE_REGISTERS: {
        let registers =
          await this.sensorDataCommandReceiver.GetPresetMultipleRegisterAnswer();
        //console.log(registers);
        this.DispatchCommandListener(
          SensorCommand.PRESET_MULTIPLE_REGISTERS,
          registers
        );
        return true;
      }
      case SensorCommand.READ_INPUT_REGISTERS: {
        let registers =
          await this.sensorDataCommandReceiver.GetInputRegistersAnswer();
        this.DispatchCommandListener(
          SensorCommand.READ_INPUT_REGISTERS,
          registers
        );
      ...
      case SensorCommand.READ_HOLDING_REGISTERS: {
        let registers =
          await this.sensorDataCommandReceiver.GetHoldingRegistersAnswer();
        this.DispatchCommandListener(
          SensorCommand.READ_HOLDING_REGISTERS,
          registers
        );
  ...
  private async SendRequesAndWaitResponse<T>(
    command: ISensorCommand
  ): Promise<T> {
    return new Promise<T>(async (resolve, reject) => {
      ...
      var interval = setTimeout(() => {
        console.warn(
          "Timeout Error. There is no data from sensor! Command: " +
            SensorCommand[command.Command]
        );
        reject();
      }, this.timeout);
      ...
      try {
        ...
        clearInterval(interval);
        console.warn(
          "Sending command error: " + +SensorCommand[command.Command]
        );
        reject(ex);
      }
    });
