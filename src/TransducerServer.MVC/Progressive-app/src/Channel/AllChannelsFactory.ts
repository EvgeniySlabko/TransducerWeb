...
import {
  CreateCellSpeedStyle,
  CreatePowerCellStyle,
  CreatetemperatureCellStyle,
  CreateTorqueCellStyle,
} from "./ChannelStyle/CellChannelStyleFactory";
import {
  CreatePowerStyle,
  CreateSpeedStyle,
  CreatetemperatureStyle,
  CreateTorqueStyle,
} from "./ChannelStyle/ChannelStyleFactory";
import { AbsoluteDataSource } from "./SensorDataSource/AbsoluteDataSource";
import {
  AbsolutePeakAnalyzer,
  PeakEventArgs,
} from "./SensorDataSource/AbsolutePeakAnalyzer";
...
import {
  DataSourseType,
  ISensorDataProvider,
} from "./SensorDataSource/ISensorDataProvider";
...
export function CreateAllChannels(
  ...
  let ScaledMainDataSource = CreateScaledDataSource(
    offsetMainDataSource,
    fullSensorInfo.valueRatio
  ); // переводим в ньютоны. Например 1kN -> 1000N
  ...
  let peackAnalizerMainDataSource =
    CreatePeackAnalizerDataSource(filterMainDataSource); // Анализатор пиков

  let cellDisplayMainDataSource =
    CreateDisplayValueDataSource(offsetMainDataSource); // Выдает данные не чаще чем fps.
  //let plotAverager = CreateAverageValueDataSource(filterDataSource, 1);

  let mainPlotChannel = CreateMainDataPlotChannel(
    alignedMainSource,
    fullSensorInfo
  );
  let mainSavingChannel = CreateMainDataPlotChannel(
    filterMainDataSource,
    fullSensorInfo
  );
  let mainCellChannel = CreateMainDataCellChannel(
    cellDisplayMainDataSource,
    fullSensorInfo
  );
  ...
  let DisplaySpeedDataSource = CreateDisplayValueDataSource(
    cutOffSpeedDataSource
  );
  let SpeedPlotChannel = CreateSpeedChannel(
    cutOffSpeedDataSource,
    fullSensorInfo
  );
  let speedCellChannel = CreateSpeedCellChannel(
    DisplaySpeedDataSource,
    fullSensorInfo
  );
  ...
  let powerPlotChannel = CreatePowerChannel(
    cutOffPowerDataSource,
    fullSensorInfo
  );
  let powerCellChannel = CreatePowerCellChannel(
    powerDisplaySource,
    fullSensorInfo
  );
  ...
  let temperatureChannel = CreateTemperatureChannel(
    temperatureDataSource,
    fullSensorInfo
  );
  let temperatureCellChannel = CreateTemperatureCellChannel(
    temperatureDataSource,
    fullSensorInfo
  );
  ...
  return {
    ...
    setAbsoluteSourceState: (absolute: boolean) =>
      (absoluteMainDataSource.Enabled = absolute),
    getAbsoluteSourceState: () => absoluteMainDataSource.Enabled,
    getInvertorSourceState: () => invertedMainDataSource.Enabled,
    setInvertiorSourceState: (invertion: boolean) =>
      (invertedMainDataSource.Enabled = invertion),
    ...
    setPeackAnalizerState: (enabled: boolean) =>
      (peackAnalizerMainDataSource.Enabled = enabled),
    getPeackAnalizerState: () => peackAnalizerMainDataSource.Enabled,
  };

  function CreateTemperatureChannel(
    source: SensorDataProvider,
    fullSensorInfo: FullSensorInfo
  ): PlotChannel {
    return new PlotChannel(source, CreatetemperatureStyle(fullSensorInfo));
  }

  function CreateCutOffDataSource(
    source: ISensorDataProvider
  ): CutOffDataSource {
    return new CutOffDataSource(source);
  }

  function CreateTemperatureCellChannel(
    source: SensorDataProvider,
    fullSensorInfo: FullSensorInfo
  ): CellChannel {
  ...
  function CreateSpeedCellChannel(
    source: ISensorDataProvider,
    fullSensorInfo: FullSensorInfo
  ): CellChannel {
  ...
  function CreateMainDataPlotChannel(
    source: ISensorDataProvider,
    fullSensorInfo: FullSensorInfo
  ): PlotChannel {
  ...
  function CreateMainDataCellChannel(
    source: ISensorDataProvider,
    fullSensorInfo: FullSensorInfo
  ): CellChannel {
  ...
  function CreateFilterDataSource(
    source: ISensorDataProvider
  ): FilterDataSource {
    return new FilterDataSource(source);
  }

  function CreatePowerCellChannel(
    source: ISensorDataProvider,
    fullSensorInfo: FullSensorInfo
  ): CellChannel {
  ...
  function CreateOffsetDataSource(
    baseSource: ISensorDataProvider
  ): OffsetDataSource {
    return new OffsetDataSource(baseSource, 0);
  }

  function CreatePeackAnalizerDataSource(
    baseSource: ISensorDataProvider
  ): AbsolutePeakAnalyzer {
    return new AbsolutePeakAnalyzer(baseSource);
  }

  function CreatePowerDataSource(
    toqueDataSourse: ISensorDataProvider,
    speedDataSourse: ISensorDataProvider
  ): PowerDataSource {
    return new PowerDataSource(toqueDataSourse, speedDataSourse);
  }

  function CreateBufferedDataSource(
    dataSource: ISensorDataProvider,
    bufferSize: number
  ): BufferSensorDataSource {
