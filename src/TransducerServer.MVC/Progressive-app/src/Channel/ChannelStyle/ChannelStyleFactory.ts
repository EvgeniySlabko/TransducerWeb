...
import {
  ...
  tmpBaseColor,
  torqueBaseColor,
} from "./ChanneStyleCommon";

const rescaleRatio: number = 0.2;
export function CreateTorqueStyle(
  ...
  return {
    ...
    line: "solid",
    range: [
      minValScaled - 0.2 * maxValScaled,
      maxValScaled + 0.2 * maxValScaled,
    ],
...
export function CreatetemperatureStyle(
  sensorInfo: FullSensorInfo
): PlotChannelStyle {
  return {
    sensorId: sensorInfo.id,
...
export function CreatePowerStyle(sensorInfo: FullSensorInfo): PlotChannelStyle {
  let minPower = CalculatePower(
    sensorInfo.MaxSpeed + 0.1 * sensorInfo.MaxSpeed,
    sensorInfo.MinValue + 0.1 * sensorInfo.MinValue
  );
